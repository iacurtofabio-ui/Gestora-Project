...
export function useDashboardGiornaliera(data: string){
   return useQuery<DashboardGiornalieraDTO>({ queryKey: ['dashboard-giornaliera', data],  // chiave univoca per la cache
...
export function useDashboardSettimanale(dataInizio: string){
   return useQuery<DashboardSettimanaleDTO>({ queryKey: ['dashboard-settimanale', dataInizio],  // chiave univoca per la cache
